...
if not st.session_state.cashier_auth:
    ...
    if search_btn:
        if not customer_phone.strip():
            ...
            if idx == -1:
                not_found_html = (
                    "<div style='background-color:#FDF3F2; border:1.5px solid #D98880; color:#78281F; "
                    "padding:14px 18px; border-radius:14px; text-align:center; font-weight:700; "
                    "margin-top:18px; box-shadow:0 4px 12px rgba(120,40,31,0.08); font-size:15px;'>"
                    "⚠️ رقم الجوال غير مسجل. اطلب من الكاشير تسجيلك في البرنامج."
                    "</div>"
                )
                st.markdown(not_found_html, unsafe_allow_html=True)
            else:
                cust = df.loc[idx]
                ...
                stamps_html = "".join(
                    f"<div style='background:{color}; color:#fff; width:38px; height:38px; border-radius:50%; "
                    f"display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:14px; "
                    f"box-shadow:0 3px 8px rgba(0,0,0,0.15);'>{i}</div>"
                    if i <= visits else
                    f"<div style='background:#F3ECE1; border:2px dashed #D9C9B7; color:#D9C9B7; width:38px; height:38px; "
                    f"border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:bold; "
                    f"font-size:14px;'>{i}</div>"
                    for i in range(1, MAX_MONTHLY_VISITS + 1)
                )

                footer_note = (
                    f"باقي لك {remaining_to_max} زيارات لتصل لفئة <b>البريميوم (40%)</b>"
                    if visits < 10
                    else "🎉 لقد وصلت للحد الأقصى واستحققت خصم 40% لهذا الشهر!"
                )

                card_html = (
                    "<div class='tier-card'>"
                    f"<div style='font-size:13px; color:#8C7B6E; letter-spacing:2px; font-weight:600;'>WELCOME, {cust['Name']}</div>"
                    ...
                    f"<div class='badge-box' style='background-color:{color};'>{emoji} الفئة الحالية: {tier_name}</div>"
                    f"<div style='font-size:16px; font-weight:700; color:#223322; margin:10px 0;'>"
                    f"نسبة الخصم الحالي: <span style='color:{color}; font-size:20px;'>{discount}</span></div>"
                    f"<div style='font-size:13px; color:#6B5C4F; margin-bottom:15px;'>"
                    f"عدد زياراتك هذا الشهر: <b>{visits} / {MAX_MONTHLY_VISITS}</b></div>"
                    f"<div style='display:flex; justify-content:center; flex-wrap:wrap; gap:8px; margin:15px 0;'>{stamps_html}</div>"
                    f"<div style='font-size:12px; color:#8C7B6E; margin-top:15px; border-top:1px solid #EADFD1; padding-top:12px;'>{footer_note}</div>"
                    "<div style='font-size:10px; color:#A38F7D; margin-top:6px; font-style:italic;'>"
                    "* تنبيه: يتم تصفير الزيارات تلقائياً مع بداية كل شهر ميلادي جديد.</div>"
                    "</div>"
                )
...
# ---------------------------------------------------------
    ...
    # -- Register new customer (Without extra PIN) --
